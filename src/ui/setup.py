...
class Setup:
    ...
    def make_body(self):
        ...
        button = Button(self.__window, text='Download',
                        command=lambda: self.download_file(entry_path_to_extract.get(),
                                                           entry_sheet.get(),
                                                           entry_file_name.get()))
    ...
    def download_file(self, path, sheet, file_name):
        path = path.strip()
        sheet = sheet.strip()
        ...
        try:
            if self.__is_empty(path):
                self.__display_error('Output path cannot be empty')
            elif self.__is_empty(file_name):
                self.__display_error('File name cannot be empty')
            elif self.__is_empty(sheet):
                self.__display_error('Sheet cannot be empty')
            else:
                try:
                    sheet = int(sheet)
                    if sheet < 0:
                        self.__display_error('Sheet number cannot be less than 0')
                    else:
                        try:
                            service = Service()
                            service.connect()
                    ...
                    self.__display_error('The number of the sheet must be a number')
        except SpreadsheetNotFound:
            self.__display_error('The file does not exists')
    ...
    def __display_error(self, text_body):
